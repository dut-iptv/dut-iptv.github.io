...
import datetime, json, os, pytz, random, string, sys, time, xbmc, xbmcplugin
...
def login(ask=True, **kwargs):
    ...
    if len(profile_settings['devicekey']) == 0:
        _devicekey = ''.join(random.choice(string.ascii_uppercase + string.ascii_lowercase + string.digits) for _ in range(64))
        ...
        for row in playdata['info']['resultObj']['containers']:
            if check_key(row, 'metadata'):
                ...
                if check_key(row['metadata'], 'pictureUrl'):
                    program_image = "{image_url}/{imgtype}/{image}/1920x1080.jpg?blurred=false".format(image_url=CONST_IMAGE_URL, imgtype=imgtype, image=row['metadata']['pictureUrl'])
